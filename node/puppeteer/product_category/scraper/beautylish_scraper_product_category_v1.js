...
function extractData($, url) {
    try {
        ...
        $(".applied-filters .filter-item").each((i, el) => {
            const name = $(el).attr("data-filter-name");
            const val = $(el).attr("data-filter-value");
            if (name && val) {
                appliedFilters.push({ filterName: name, filterValue: val });
            }
        });
        ...
        if (appliedFilters.length > 0) {
            catID = appliedFilters[0].filterValue;
        }
        outputData.categoryId = catID;

        let catName = $("h1.category-name").first().text().trim();
        if (!catName) {
            catName = $(".category-header h1").first().text().trim();
        }
        if (!catName) {
            catName = $("title").first().text().replace(" | Beautylish", "").trim();
        }
        outputData.categoryName = catName;

        let catUrl = $("link[rel='canonical']").attr("href") || "";
        outputData.categoryUrl = makeAbsoluteURL(catUrl);
        outputData.description = null;
        outputData.pagination = null;

        const products = [];
        $("div[data-product-id], .product-grid-item, .product-card").each((i, el) => {
            const s = $(el);
            const product = {};

            let name = s.find(".product-name, .name, [itemprop='name']").first().text().trim();
            if (!name) {
                name = s.attr("data-product-name") || "";
            }
            product.name = name;

            product.brand = s.find(".brand-name, .brand, [itemprop='brand']").first().text().trim();

            const priceText = s.find(".price, .product-price, [itemprop='price']").first().text().trim();
            product.price_text = priceText;
            product.currency = detectCurrency(priceText);

            let pUrl = s.find("a").first().attr("href") || "";
            product.url = makeAbsoluteURL(pUrl);

            let imgSrc = "";
            const img = s.find("img").first();
            imgSrc = img.attr("src") || img.attr("data-src") || "";
            product.image = makeAbsoluteURL(imgSrc);
        ...
        $(".squeezeBox_sublist .squeezeBox_item").each((i, el) => {
            const link = $(el).find("a.squeezeBox_link").first();
...
async function scrapePage(url, pipeline, browser, retries = CONFIG.maxRetries) {
    ...
    while (attempts <= retries && !success) {
        ...
        try {
            ...
            // Wait for product grid elements or category headers as defined in Go code
            try {
                await page.waitForSelector('.product-grid-item, .category-name, div[data-product-id]', { timeout: 10000 });
            } catch (e) {
                console.log('Wait selector timeout, proceeding with current DOM');
            }
